...
(function ($) {
  ...
  function calendar($el, options) {
    ...
    function monthAddEvent(index, event) {
      var $event = $('<div/>', {'class': 'event status bg-info', text: event.title, title: event.title, 'data-index': index}),
          ...
          existing,
          i;
      $event.toggleClass('bg-alert', !!event.allDay);
      if (!!time) {
        $event.html('<strong>' + time + '</strong> ' + $event.html());
      }
      if (!event.end) {
        $event.addClass('begin bg-success end');
        $event.removeClass('bg-info');
  ...
  ;(function (defaults, $, window, document) {
    ...
    shortMonths: ["Jan", "Feb", "Mar", "Abr", "Mai", "Jun", "Jul", "Aug", "Set", "Out", "Nov", "Dez"],
    date: (new Date()),
...
var data = [],
    ...
    h,
    m,
    names = ['Reunião', 'Curso', 'Audiência'],
    slipsum = ["Mussum Ipsum, cacilds vidis litro abertis. Posuere libero varius. Nullam a nisl ut ante blandit hendrerit. Aenean sit amet nisi. Leite de capivaris, leite de mula manquis sem cabeça. A ordem dos tratores não altera o pão duris. Delegadis gente finis, bibendum egestas augue arcu ut est."];
